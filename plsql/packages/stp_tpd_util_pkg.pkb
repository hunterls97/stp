...
AS
    ...
    l_type_id               STP_TREE_PLANTING_DETAIL_ROW.ACTIVITY_TYPE_ID%TYPE         := APEX_UTIL.GET_SESSION_STATE('P43_TYPE_ID'); 
    l_stock_type_id         STP_TREE_PLANTING_DETAIL_ROW.STOCK_TYPE_ID%TYPE            := APEX_UTIL.GET_SESSION_STATE('P43_STOCK_TYPE');
    l_plant_type_id         STP_TREE_PLANTING_DETAIL_ROW.PLANT_TYPE_ID%TYPE            := APEX_UTIL.GET_SESSION_STATE('P43_PLANT_TYPE');
    l_species_id            STP_TREE_PLANTING_DETAIL_ROW.SPECIES_ID%TYPE               := APEX_UTIL.GET_SESSION_STATE('P43_SPECIES');
    l_stumping_size_id      STP_TREE_PLANTING_DETAIL_ROW.STUMP_SIZE_ID%TYPE            := APEX_UTIL.GET_SESSION_STATE('P43_STUMPING_SIZE');
    l_transp_dis_id         STP_TREE_PLANTING_DETAIL_ROW.TRANSPLANTING_DISTANCE_ID%TYPE:= APEX_UTIL.GET_SESSION_STATE('P43_TRANSP_DIS');
    l_quantity              STP_TREE_PLANTING_DETAIL_ROW.QTY%TYPE                      := APEX_UTIL.GET_SESSION_STATE('P43_QUANTITY');
    l_id                    STP_TREE_PLANTING_DETAIL_ROW.ID%TYPE                       := APEX_UTIL.GET_SESSION_STATE('P43_ID');
    l_mark_type_id          STP_TREE_PLANTING_DETAIL_ROW.MARK_TYPE_ID%TYPE             := APEX_UTIL.GET_SESSION_STATE('P43_MARK_TYPE');
    l_marking_location_id   STP_TREE_PLANTING_DETAIL_ROW.MARKING_LOCATION_ID%TYPE      := APEX_UTIL.GET_SESSION_STATE('P43_MARKING_LOCATION');
    l_offset_from_mark      STP_TREE_PLANTING_DETAIL_ROW.OFFSET_FROM_MARK%TYPE         := APEX_UTIL.GET_SESSION_STATE('P43_OFFSET');
    l_spacing_on_centre     STP_TREE_PLANTING_DETAIL_ROW.SPACING_ON_CENTRE%TYPE        := APEX_UTIL.GET_SESSION_STATE('P43_SPACE');
    l_hydro                 STP_TREE_PLANTING_DETAIL_ROW.HYDRO%TYPE                    := APEX_UTIL.GET_SESSION_STATE('P43_HYDRO');
    l_rin                   STP_TREE_PLANTING_DETAIL_ROW.RIN%TYPE                      := APEX_UTIL.GET_SESSION_STATE('P43_RIN');
    l_roadside              STP_TREE_PLANTING_DETAIL_ROW.ROADSIDE%TYPE                 := APEX_UTIL.GET_SESSION_STATE('P43_ROADSIDE');
    l_between_road_1        STP_TREE_PLANTING_DETAIL_ROW.BETWEEN_ROAD_1%TYPE           := APEX_UTIL.GET_SESSION_STATE('P43_BETWEEN_ROAD_1');
    l_between_road_2        STP_TREE_PLANTING_DETAIL_ROW.BETWEEN_ROAD_2%TYPE           := APEX_UTIL.GET_SESSION_STATE('P43_BETWEEN_ROAD_2');
    l_address               STP_TREE_PLANTING_DETAIL_ROW.ADDRESS%TYPE                  := APEX_UTIL.GET_SESSION_STATE('P43_ADDRESS');
    l_comments              STP_TREE_PLANTING_DETAIL_ROW.COMMENTS%TYPE                 := APEX_UTIL.GET_SESSION_STATE('P43_COMMENTS');
...
BEGIN
    ...
    l_stock_type       := STP_UTIL_PKG.LOAD_PARAMETER(STP_CONSTANT_PKG.GC_
